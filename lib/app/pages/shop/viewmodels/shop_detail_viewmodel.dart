...
class ShopDetailViewModel extends ChangeNotifier {
  ...
  // 默认地址数据
  UserAddressEntity? _defaultAddress;
  ...
  UserAddressEntity? get defaultAddress => _defaultAddress;
  ...
  /// 更新地址
  void updateAddress(UserAddressEntity? address) {
    _defaultAddress = address;
    notifyListeners();
  }

  /// 确保有购物车ID（用于立即购买）
  Future<String?> ensureCartIdForBuyNow({
    required String productId,
  }) async {
    if ((_cartId ?? '').isNotEmpty) return _cartId;
    final ok = await addToCart(cartNum: 1, productId: productId);
    return ok ? _cartId : null;
  }
